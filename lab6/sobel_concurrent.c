...
void sobel_gpu(const CImg<float>& image, CImg<float>& output, 
                                          const std::vector<std::vector<float>>& kernel_gx, 
                                          const std::vector<std::vector<float>>& kernel_gy) {
    int rows = image.height();
    int cols = image.width();
...
int sobel_cpu_batch(const vector<CImg<float>>& images, vector<CImg<float>>& outputs,
...
int sobel_gpu_batch(const vector<CImg<float>>& images, vector<CImg<float>>& outputs,
                     const vector<vector<float>>& kernel_gx, const vector<vector<float>>& kernel_gy,
                     atomic<int>& global_execution_count, double& total_time, int& local_count, int max_images) {
    while (global_execution_count.load() < max_images) {
        for (size_t i = 0; i < images.size() && global_execution_count.load() < max_images; ++i) {
            auto start = std::chrono::high_resolution_clock::now();
            sobel_gpu(images[i], outputs[i], kernel_gx, kernel_gy);
            auto end = std::chrono::high_resolution_clock::now();
...
int main(int argc, char** argv) {
    ...
    if (image.spectrum() > 1) {
        image = image.RGBtoYCbCr().channel(0); // Convertir a escala de grises
    }

    const int total_images_to_process = 5000;
    ...
    thread gpu_thread(sobel_gpu_batch, ref(image_stream), ref(gpu_outputs), ref(kernel_gx), ref(kernel_gy),
                      ref(global_execution_count), ref(gpu_total_time), ref(gpu_execution_count), total_images_to_process);
